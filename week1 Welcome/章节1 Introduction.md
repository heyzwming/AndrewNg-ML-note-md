...
Given data about the size of houses on the real estate(房地产) market, try to predict their price. Price as a function of size(价格是房子大小有关的函数) is a **continuous output**, so this is a **regression problem**.

在例子中，横轴(horizontal axis)是不同房屋的平方英尺数,纵轴(vertical axis)是不同房子的价格，单位是千美元,如果你进行的是线性回归，(或者说用一条直线拟合数据)那么面积为750的房子，得到的是150k的价格，但是，线性模型似乎与实际数据的拟合度不是特别好。如果采用二次函数进行拟合，可以看到拟合度更高了，得到了200k的价格。这样看来，回归问题的模型选取是十分重要的。
...
The other way,we could turn this example into a **classification problem** by instead making our output about whether the house "sells for more or less than the asking price." Here we are classifying the houses based on price into two discrete(离散的) categories(分类).
